...
class bert_ATE(torch.nn.Module):
    ...
    def forward(self, ids_tensors, tags_tensors, masks_tensors):
        ...
        linear_outputs = self.linear(self.bert(input_ids=ids_tensors, attention_mask=masks_tensors).last_hidden_state)
...
class bert_ABSA(torch.nn.Module):
    ...
    def forward(self, ids_tensors, lable_tensors, masks_tensors, segments_tensors):
        ...
        linear_outputs = self.linear(self.bert(input_ids=ids_tensors, attention_mask=masks_tensors,token_type_ids=segments_tensors).pooler_output)	
